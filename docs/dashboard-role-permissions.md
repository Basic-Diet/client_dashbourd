...
- `cashier`: operations, manual deduction, and customer lifecycle under users.
...
| `/api/dashboard/overview`, `/api/dashboard/search`, `/api/dashboard/users`, `/api/dashboard/orders`, `/api/dashboard/payments`, subscription read/create/quote routes in `admin.js` | `superadmin`, `admin`, `cashier` | Cashier dashboard, customer, order, subscription, and payment read/workflow routes where backend permits. Backend authorization must be verified separately from frontend controls. |
| `/api/dashboard/plans` GET, `/api/dashboard/addons` GET, `/api/admin/builder-premium-meals` GET, `/api/dashboard/settings` GET, `/api/dashboard/zones` GET | Backend dependency for `superadmin`, `admin`, `cashier` read access | Exact read-only catalog endpoints currently used by the customer subscription form. Cashier may select an existing package during customer subscription creation, but package management and package CRUD remain forbidden. Add-on, premium-meal, settings, and zone writes remain forbidden. |
| `/api/dashboard/menu*`, `/api/dashboard/catalog-items*`, `/api/dashboard/premium-upgrades*` | `superadmin`, `admin`, `kitchen` | Kitchen owns menu/catalog/premium upgrade admin surfaces according to the latest backend role contract. |
| `/api/dashboard/settings*` write routes, `/api/dashboard/restaurant-hours*`, `/api/dashboard/promo-codes*`, `/api/dashboard/dashboard-users*`, `/api/dashboard/notifications*`, `/api/dashboard/logs*` | `superadmin`, `admin` | Admin configuration and management APIs. Dashboard user management remains admin/superadmin unless product later decides superadmin-only. |
| `/api/dashboard/accounting/*` | `superadmin`, `admin` | Accounting screen is not cashier-accessible unless explicitly approved later. |

## Customer Lifecycle

`superadmin`, `admin`, and `cashier` have customer lifecycle frontend controls from `/users`, including search/filter, create customer, view the one-time temporary credentials returned by create/reset responses, view customer details, activate/deactivate app customers, reset eligible app-customer passwords, and start a subscription from `/users/:userId/create-subscription`.

Cashier subscription creation from a customer page returns to `/users/:userId` after success. Cashier does not get the standalone `/subscriptions` route tree and does not get admin pages such as `/packages`, `/dashboard-users`, `/settings`, `/accounting`, `/menu`, `/addons`, `/promo-codes`, or `/zones`.

Package selection during customer subscription creation is allowed through the subscription form only. Package management pages and package CRUD are forbidden for cashier. Dashboard staff-user management under `/dashboard-users` is a separate staff-account system and remains forbidden for cashier.

Backend endpoint authorization for the cashier customer lifecycle is a separate backend dependency and must not be assumed complete from the frontend route controls alone.

Customer temporary passwords are generated by the backend and returned once by create/reset mutations only. The frontend must not place temporary passwords in URLs, browser storage, persistent query cache, or logs. Cashier password reset applies only to app customers; dashboard staff password reset remains outside the cashier surface.
