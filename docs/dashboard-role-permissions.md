...
| `/api/dashboard/plans` GET, `/api/dashboard/plans/:id` GET, `/api/dashboard/addons*` GET, `/api/dashboard/addon-plans*` GET, `/api/dashboard/addon-items*` GET, `/api/dashboard/addon-prices*` GET, `/api/dashboard/builder-premium-meals*` GET, `/api/dashboard/settings` GET, `/api/dashboard/zones*` GET | `superadmin`, `admin`, `cashier`, plus `kitchen` for package/add-on catalog reads | Exact read-only endpoints needed by the customer subscription form. Package/add-on/settings/zone writes remain admin-only. |
