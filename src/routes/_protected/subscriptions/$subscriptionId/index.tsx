...
import { subscriptionDetailsQueryOptions, useUnfreezeSubscriptionMutation } from "@/hooks/useSubscriptionsQuery";
...
export const Route = createFileRoute(
  ...
  loader: ({ context: { queryClient }, params }: any) =>
...
function SubscriptionDetailsPage() {
  ...
  const { mutateAsync: unfreezeSubscription } = useUnfreezeSubscriptionMutation();
  ...
  const handleUnfreeze = async () => {
    try {
      ...
      console.error(error);
      toast.error("حدث خطأ أثناء إلغاء تجميد الاشتراك");
    }
  };

  return (
    <div className="flex-1 space-y-6 pt-4 px-2">
      <SubscriptionHeader 
