...
import { CategoriesTable } from "@/components/pages/categories/categories-table";
...
export const Route = createFileRoute("/_protected/categories/")({
  ...
  pendingComponent: () => (
    <Loader variant="full-screen" label="جاري تحميل التصنيفات..." />
  ),
});

function RouteComponent() {
  const { data: categoriesResponse } = useSuspenseQuery(categoriesQueryOptions());
            ...
            <div className="flex items-center gap-4">
              ...
              <div className="space-y-1">
                <h2 className="text-xl font-bold tracking-tight">
                  تصنيفات الوجبات (Meal Categories)
                </h2>
                <p className="text-sm text-muted-foreground">
                  إدارة تصنيفات الوجبات والتعديل عليها
                </p>
              </div>
            </div>
